    ...
    <template #footer>
      <div class="flex flex-nowrap items-center justify-between mt-40">
        ...
        <nuxt-link :to="localePath('/contact')"
          class="p-2 sm:p-4 border-zebraBorder border-opacity-20 rounded-sm text-white border-2 mb-10">{{ $t('contact') }}</nuxt-link>
        <nuxt-link to='https://shop.zebratbilisi.com/'
          class="p-2 sm:p-4 border-zebraBorder border-opacity-20 rounded-sm text-white border-2 mb-10">{{$t('getYourTickets')}}</nuxt-link>
    </div>
    <zebra-footer />
  </template>
</NuxtLayout></template>