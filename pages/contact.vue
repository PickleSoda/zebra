...
<template>
        ...
        <div class="mx-8 p-2 md:p-8 border-2 border-zebraBorder border-opacity-80 rounded-sm">
            ...
            <div class="flex flex-col gap-3">
                ...
                <textarea name="message" cols="30" rows="8"
                    class="bg-zebraBackground border-2 border-zebraBorder  p-2 text-white" placeholder="message"></textarea>
            </div>
        </div>
    ...
    <div class="relative transform translate-y-1/2 z-10 px-8 w-full max-w-5xl">
        <div class="flex flex-nowrap items-center justify-between">
            ...
            <button
                class="p-2 border-zebraBorder border-opacity-20 rounded-sm text-white border-2  mb-10 md:mb-0 bg-zebraBackground">{{ $t('sendToZebra') }}</button>
            <button
                class="p-2 border-zebraBorder border-opacity-20 rounded-sm text-white border-2  mb-10 md:mb-0 bg-zebraBackground">{{ $t('sendToSupport') }}</button>
        </div>
        <zebra-footer />
</div></template>